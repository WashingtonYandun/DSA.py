...
# x = [[int(c) for c in r.split(" ")] for r in x.split("\n")]


# named A cause Linear Algebra memories c':
A = [
        ...
        for j in range(cols - n + 1):  # 1 added for 0 in arrays
            ...
            if i < rows - n + 1:
                ...
                currentProdDia = max(
                    ...
                    A[i][j+3]*A[i+1][j+2] * A[i+2][j+1] *
                    A[i+3][j]  # Second Diagonal
                )
                target = max(currentProdDia, target)
    return target


print(largest_product_in_a_grid(A, 4))  # 70600674
